...
Hooks 提供了一个可扩展的事件驱动系统，用于在响应智能体命令和事件时自动执行操作。Hooks 会从目录中自动发现，并且可以通过 CLI 命令进行管理，方式与 FoxFang 中的 Skills 类似。
...
- **Webhooks**：外部 HTTP webhook，可让其他系统在 FoxFang 中触发工作。请参阅 [Webhook Hooks](/automation/webhook)，或使用 `foxfang webhooks` 获取 Gmail 辅助命令。
...
- 记录所有命令以供审计
- 在智能体生命周期事件上触发自定义自动化
- 在不修改核心代码的情况下扩展 FoxFang 的行为

## 入门指南

### 内置 Hooks

FoxFang 自带四个会被自动发现的内置 hook：
...
在新手引导期间（`foxfang onboard`），系统会提示你启用推荐的 hooks。向导会自动发现符合条件的 hooks 并供你选择。
...
3. **内置 hooks**：`<foxfang>/dist/hooks/bundled/`（随 FoxFang 一起提供）
...
Hook 包是标准的 npm 包，它们通过 `package.json` 中的 `foxfang.hooks` 导出一个或多个 hook。使用以下命令安装它们：

```bash
foxfang hooks install <path-or-spec>
```

npm spec 仅支持注册表形式（包名 + 可选的精确版本或 dist-tag）。
Git/URL/file spec 和 semver 范围会被拒绝。

裸 spec 和 `@latest` 会保持在稳定轨道上。如果 npm 将其中任意一种解析为预发布版本，FoxFang 会停止并要求你通过预发布标签（例如 `@beta`/`@rc`）或精确的预发布版本显式选择加入。
...
每个 `foxfang.hooks` 条目在解析符号链接后都必须保持在包目录内部；超出目录范围的条目会被拒绝。

安全说明：`foxfang hooks install` 会使用 `npm install --ignore-scripts` 安装依赖
...
这些 hooks 不是事件流监听器；它们允许插件在 FoxFang 持久化工具结果之前同步调整工具结果。
...
```yaml
metadata: { "foxfang": { "events": ["command:new"] } } # 精确
```

而不是：

```yaml
metadata: { "foxfang": { "events": ["command"] } } # 通用 - 开销更大
```

...
- 认证配置文件（OAuth + API 密钥 + 可选的值级引用）：`~/.foxfang/agents/<agentId>/agent/auth-profiles.json`
- 旧版兼容文件：`~/.foxfang/agents/<agentId>/agent/auth.json`
  （发现静态 `api_key` 条目时会进行清理）

仅用于旧版导入的文件（仍受支持，但不是主存储）：

- `~/.foxfang/credentials/oauth.json`（首次使用时会导入到 `auth-profiles.json`）

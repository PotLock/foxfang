...
- **会话存储**（聊天历史 + 路由状态）位于 `~/.foxfang/agents/<agentId>/sessions` 下。
...
Skills 通过每个工作区的 `skills/` 文件夹实现每智能体独立，共享的 Skills 可从 `~/.foxfang/skills` 获取。参见 [Skills：每智能体 vs 共享](/tools/skills#per-agent-vs-shared-skills)。
...
- 配置：`~/.foxfang/openclaw.json`（或 `OPENCLAW_CONFIG_PATH`）
- 状态目录：`~/.foxfang`（或 `OPENCLAW_STATE_DIR`）
- 工作区：`~/.foxfang/workspace`（或 `~/.foxfang/workspace-<agentId>`）
- 智能体目录：`~/.foxfang/agents/<agentId>/agent`（或 `agents.list[].agentDir`）
...
- 工作区默认为 `~/.foxfang/workspace`（或当设置了 `OPENCLAW_PROFILE` 时为 `~/.foxfang/workspace-<profile>`）。
- 状态默认为 `~/.foxfang/agents/main/agent`。
...
```js
{
  ...
  channels: {
    whatsapp: {
      accounts: {
        personal: {
          // 可选覆盖。默认：~/.foxfang/credentials/whatsapp/personal
          // authDir: "~/.foxfang/credentials/whatsapp/personal",
        },
        biz: {
          // 可选覆盖。默认：~/.foxfang/credentials/whatsapp/biz

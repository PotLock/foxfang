...
grep -n "agents\\|groupChat\\|mentionPatterns\\|channels\\.whatsapp\\.groups\\|channels\\.telegram\\.groups\\|channels\\.imessage\\.groups\\|channels\\.discord\\.guilds" \
  "${OPENCLAW_CONFIG_PATH:-$HOME/.foxfang/openclaw.json}"
```
...
trash "${OPENCLAW_STATE_DIR:-$HOME/.foxfang}/credentials" # 如果 logout 无法完全清除所有内容
...
```bash
# 在配置中打开跟踪日志：
#   ${OPENCLAW_CONFIG_PATH:-$HOME/.foxfang/openclaw.json} -> { logging: { level: "trace" } }
#
...
| Gateway 网关服务日志（监管程序） | macOS：`$OPENCLAW_STATE_DIR/logs/gateway.log` + `gateway.err.log`（默认：`~/.foxfang/logs/...`；配置文件使用 `~/.foxfang-<profile>/logs/...`）<br />Linux：`journalctl --user -u openclaw-gateway[-<profile>].service -n 200 --no-pager`<br />Windows：`schtasks /Query /TN "OpenClaw Gateway (<profile>)" /V /FO LIST` |
...
trash "${OPENCLAW_STATE_DIR:-$HOME/.foxfang}"

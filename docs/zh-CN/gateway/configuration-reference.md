---
description: Complete field-by-field reference for ~/.foxfang/openclaw.json
read_when:
  - 你需要精确到字段级别的配置语义或默认值
...
`~/.foxfang/openclaw.json` 中所有可用字段。若需面向任务的概览，请参见 [Configuration](/gateway/configuration)。
...
OpenClaw 使用 pi-coding-agent 模型目录。可通过配置中的 `models.providers` 或 `~/.foxfang/agents/<agentId>/agent/models.json` 添加自定义提供商。

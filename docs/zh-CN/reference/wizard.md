...
<Steps>
  ...
  <Step title="模型/认证">
    ...
    - OAuth 凭证保存在 `~/.foxfang/credentials/oauth.json`；auth-profile 保存在 `~/.foxfang/agents/<agentId>/agent/auth-profiles.json`（API key + OAuth）。
...
- 下载适合的发布资源。
- 将其存储到 `~/.foxfang/tools/signal-cli/<version>/` 下。

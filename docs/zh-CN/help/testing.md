...
- 配置文件存储：`~/.foxfang/credentials/`（首选；测试中"配置文件密钥"的含义）
...
- `OPENCLAW_CONFIG_DIR=...`（默认：`~/.foxfang`）挂载到 `/home/node/.foxfang`
- `OPENCLAW_WORKSPACE_DIR=...`（默认：`~/.foxfang/workspace`）挂载到 `/home/node/.foxfang/workspace`

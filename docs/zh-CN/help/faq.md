...
1. 在新机器上安装 OpenClaw。
2. 从旧机器复制 `$OPENCLAW_STATE_DIR`（默认：`~/.foxfang`）。
...
使用托管覆盖而不是编辑仓库副本。将你的更改放在 `~/.foxfang/skills/<name>/SKILL.md`（或通过 `~/.foxfang/openclaw.json` 中的 `skills.load.extraDirs` 添加文件夹）。优先级是 `<workspace>/skills` > `~/.foxfang/skills` > 内置，所以托管覆盖优先生效而不会修改 git。只有值得上游合并的编辑才应该放在仓库中并作为 PR 提交。

### 可以从自定义文件夹加载 Skills 吗

可以。通过 `~/.foxfang/openclaw.json` 中的 `skills.load.extraDirs` 添加额外目录（最低优先级）。默认优先级保持不变：`<workspace>/skills` → `~/.foxfang/skills` → 内置 → `skills.load.extraDirs`。`clawhub` 默认安装到 `./skills`，OpenClaw 将其视为 `<workspace>/skills`。
...
ClawHub 安装到当前目录下的 `./skills`（或回退到你配置的 OpenClaw 工作区）；OpenClaw 在下一个会话中将其视为 `<workspace>/skills`。对于跨智能体共享的 Skills，将它们放在 `~/.foxfang/skills/<name>/SKILL.md`。某些 Skills 期望通过 Homebrew 安装二进制文件；在 Linux 上意味着 Linuxbrew（参阅上面的 Homebrew Linux 常见问题条目）。参阅[Skills](/tools/skills)和 [ClawHub](/tools/clawhub)。
...
所有内容位于 `$OPENCLAW_STATE_DIR`（默认：`~/.foxfang`）下：
...
旧版单智能体路径：`~/.foxfang/agent/*`（通过 `openclaw doctor` 迁移）。
...
OpenClaw 从 `$OPENCLAW_CONFIG_PATH`（默认：`~/.foxfang/openclaw.json`）读取可选的 **JSON5** 配置：
...
- 从备份恢复（git 或复制的 `~/.foxfang/openclaw.json`）。
...
- 当前工作目录下的 `.env`
- `~/.foxfang/.env`（即 `$OPENCLAW_STATE_DIR/.env`）的全局回退 `.env`
...
- 如果你使用了配置文件（`--profile` / `OPENCLAW_PROFILE`），重置每个状态目录（默认为 `~/.foxfang-<profile>`）。
...
- 编辑 `~/.foxfang/openclaw.json` 中的 `agents.defaults.model`
...
- **确认认证配置文件的位置**（新路径 vs 旧路径）
  ...
  - 旧版：`~/.foxfang/agent/*`（通过 `openclaw doctor` 迁移）
...
- 每实例使用 `openclaw --profile <name> …`（自动创建 `~/.foxfang-<name>`）。
...
- macOS：`$OPENCLAW_STATE_DIR/logs/gateway.log` 和 `gateway.err.log`（默认：`~/.foxfang/logs/...`；配置文件使用 `~/.foxfang-<profile>/logs/...`）

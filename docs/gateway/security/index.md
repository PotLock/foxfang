...
- If someone can modify Gateway host state/config (`~/.foxfang`, including `openclaw.json`), treat them as a trusted operator.
...
| ------------------------------------------------------------- | ------------- | ------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------- | -------- |
| `fs.state_dir.perms_world_writable`                           | critical      | Other users/processes can modify full OpenClaw state                                 | filesystem perms on `~/.foxfang`                                                                    | yes      |
| `fs.config.perms_writable`                                    | critical      | Others can change auth/tool policy/config                                            | filesystem perms on `~/.foxfang/openclaw.json`                                                      | yes      |
...
OpenClaw stores session transcripts on disk under `~/.foxfang/agents/<agentId>/sessions/*.jsonl`.
...
- If you install plugins (`openclaw plugins install <package>`), treat it like running untrusted code:
  - The install path is `~/.foxfang/extensions/<pluginId>/` (or `$OPENCLAW_STATE_DIR/extensions/<pluginId>/`).
...
- `~/.foxfang/openclaw.json`: `600` (user read/write only)
...
Assume anything under `~/.foxfang/` (or `$OPENCLAW_STATE_DIR/`) may contain secrets or private data:

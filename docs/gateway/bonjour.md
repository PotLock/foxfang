...
- serve your chosen domain (example: `openclaw.internal.`) from `~/.foxfang/dns/<domain>.db`
...
- Set `gateway.bind: "tailnet"` in `~/.foxfang/openclaw.json`.
...
- `gateway.bind` in `~/.foxfang/openclaw.json` controls the Gateway bind mode.

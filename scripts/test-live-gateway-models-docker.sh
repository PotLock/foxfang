...
elif [[ -n "${FOXFANG_LIVE_GATEWAY_PROVIDERS:-}" ]]; then
  while IFS= read -r auth_dir; do
    [[ -n "$auth_dir" ]] || continue
    AUTH_DIRS+=("$auth_dir")
  done < <(foxfang_live_collect_auth_dirs_from_csv "${FOXFANG_LIVE_GATEWAY_PROVIDERS:-}")
else
  while IFS= read -r auth_dir; do
    [[ -n "$auth_dir" ]] || continue
    AUTH_DIRS+=("$auth_dir")
...
docker run --rm -t \
  ...
  -e FOXFANG_LIVE_GATEWAY_MODELS="${FOXFANG_LIVE_GATEWAY_MODELS:-modern}" \
  -e FOXFANG_LIVE_GATEWAY_PROVIDERS="${FOXFANG_LIVE_GATEWAY_PROVIDERS:-}" \
  -e FOXFANG_LIVE_GATEWAY_MAX_MODELS="${FOXFANG_LIVE_GATEWAY_MAX_MODELS:-24}" \
  -e FOXFANG_LIVE_GATEWAY_MODEL_TIMEOUT_MS="${FOXFANG_LIVE_GATEWAY_MODEL_TIMEOUT_MS:-}" \
  -v "$ROOT_DIR":/src:ro \

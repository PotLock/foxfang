...
import { discoverFoxFangPlugins } from "../src/plugins/discovery.js";
...
const ROOT_IMPORT_PATTERN = /["']foxfang\/plugin-sdk["']/;
const LEGACY_COMPAT_IMPORT_PATTERN = /["']foxfang\/plugin-sdk\/compat["']/;
...
function main() {
  ...
  if (monolithicOffenders.length > 0 || legacyCompatOffenders.length > 0) {
    if (monolithicOffenders.length > 0) {
      console.error("Bundled plugin source files must not import monolithic foxfang/plugin-sdk.");
    ...
    if (legacyCompatOffenders.length > 0) {
      console.error(
        "Bundled plugin source files must not import legacy foxfang/plugin-sdk/compat.",
      );
    ...
    if (monolithicOffenders.length > 0 || legacyCompatOffenders.length > 0) {
      console.error(
        "Use foxfang/plugin-sdk/<domain> or foxfang/plugin-sdk/<channel> subpaths for bundled plugins; root and compat are legacy surfaces only.",
      );
    }
    process.exit(1);
  }

...
# Build and bundle FoxFang into a minimal .app we can open.
# Outputs to dist/FoxFang.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/FoxFang.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="FoxFang"
BUNDLE_ID="${BUNDLE_ID:-ai.foxfang.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/foxfang/foxfang/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/FoxFang/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :FoxFangBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :FoxFangGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/FoxFang"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/FoxFang"
fi
chmod +x "$APP_ROOT/Contents/MacOS/FoxFang"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/FoxFang" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/FoxFang/Resources/FoxFang.icns" "$APP_ROOT/Contents/Resources/FoxFang.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/FoxFang/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying FoxFangKit resources"
FOXFANGKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/FoxFangKit_FoxFangKit.bundle"
if [ -d "$FOXFANGKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/FoxFangKit_FoxFangKit.bundle"
  cp -R "$FOXFANGKIT_BUNDLE" "$APP_ROOT/Contents/Resources/FoxFangKit_FoxFangKit.bundle"
else
  echo "WARN: FoxFangKit resource bundle not found at $FOXFANGKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running FoxFang"
killall -q FoxFang 2>/dev/null || true

...
ZIP=${1:?"Usage: $0 FoxFang-<ver>.zip"}
FEED_URL=${2:-"https://raw.githubusercontent.com/foxfang/foxfang/main/appcast.xml"}
...
if [[ -z "$VERSION" ]]; then
  ...
  if [[ "$ZIP_NAME" =~ ^FoxFang-([0-9]+(\.[0-9]+){1,2}([-.][0-9A-Za-z]+([.-][0-9A-Za-z]+)*)?)\.zip$ ]]; then

...
usage() {
  cat <<'EOF'
Usage:
  FOXFANG_PUSH_RELAY_BASE_URL=https://relay.example.com \
...
- regenerates apps/ios/FoxFang.xcodeproj via xcodegen
EOF
}
...
PUSH_RELAY_BASE_URL="${FOXFANG_PUSH_RELAY_BASE_URL:-${IOS_PUSH_RELAY_BASE_URL:-}}"
...
validate_push_relay_base_url() {
  ...
  if [[ "${value}" =~ [[:space:]] ]]; then
    echo "Invalid FOXFANG_PUSH_RELAY_BASE_URL: whitespace is not allowed." >&2
    exit 1
  fi

  if [[ "${value}" == *'$'* || "${value}" == *'('* || "${value}" == *')'* || "${value}" == *'='* ]]; then
    echo "Invalid FOXFANG_PUSH_RELAY_BASE_URL: contains forbidden xcconfig characters." >&2
    exit 1
  fi

  if [[ ! "${value}" =~ ^https://[A-Za-z0-9.-]+(:([0-9]{1,5}))?(/[A-Za-z0-9._~!&*+,;:@%/-]*)?$ ]]; then
    echo "Invalid FOXFANG_PUSH_RELAY_BASE_URL: expected https://host[:port][/path]." >&2
    exit 1
  fi

  local port="${BASH_REMATCH[2]:-}"
  if [[ -n "${port}" ]] && (( 10#${port} > 65535 )); then
    echo "Invalid FOXFANG_PUSH_RELAY_BASE_URL: port must be between 1 and 65535." >&2
    exit 1
  fi
}
...
if [[ -z "${PUSH_RELAY_BASE_URL}" ]]; then
  echo "Missing FOXFANG_PUSH_RELAY_BASE_URL (or IOS_PUSH_RELAY_BASE_URL) for beta relay registration." >&2
  exit 1
fi
...
PUSH_RELAY_BASE_URL_XCCONFIG="$(
  printf '%s' "${PUSH_RELAY_BASE_URL}" \
    | sed 's#//#$(FOXFANG_URL_SLASH)$(FOXFANG_URL_SLASH)#g'
)"
...
FOXFANG_CODE_SIGN_STYLE = Automatic
FOXFANG_DEVELOPMENT_TEAM = ${TEAM_ID}
FOXFANG_IOS_SELECTED_TEAM = ${TEAM_ID}
FOXFANG_APP_BUNDLE_ID = ai.foxfang.client
FOXFANG_SHARE_BUNDLE_ID = ai.foxfang.client.share
FOXFANG_ACTIVITY_WIDGET_BUNDLE_ID = ai.foxfang.client.activitywidget
FOXFANG_WATCH_APP_BUNDLE_ID = ai.foxfang.client.watchkitapp
FOXFANG_WATCH_EXTENSION_BUNDLE_ID = ai.foxfang.client.watchkitapp.extension
FOXFANG_APP_PROFILE =
FOXFANG_SHARE_PROFILE =
FOXFANG_PUSH_TRANSPORT = relay
FOXFANG_PUSH_DISTRIBUTION = official
FOXFANG_URL_SLASH = /
FOXFANG_PUSH_RELAY_BASE_URL = ${PUSH_RELAY_BASE_URL_XCCONFIG}
FOXFANG_PUSH_APNS_ENVIRONMENT = production
EOF

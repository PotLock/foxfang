...
# FoxFang Auth Widget for Termux
...
SERVER="${FOXFANG_SERVER:-l36}"

# Check auth status
termux-toast "Checking FoxFang auth..."

STATUS=$(ssh "$SERVER" '$HOME/foxfang/scripts/claude-auth-status.sh simple' 2>&1)
EXIT_CODE=$?

case "$STATUS" in
    OK)
        # Get remaining time
        DETAILS=$(ssh "$SERVER" '$HOME/foxfang/scripts/claude-auth-status.sh json' 2>&1)
    ...
    CLAUDE_EXPIRING|FOXFANG_EXPIRING)
...
3. SSH to server and paste key"
                ...
                termux-toast "Run: ssh $SERVER '$HOME/foxfang/scripts/mobile-reauth.sh'"
                ;;
            *)
    ...
    CLAUDE_EXPIRED|FOXFANG_EXPIRED)
...
2. Return here and tap OK to SSH"
                ...
                ;;
            *)
                termux-toast "Warning: FoxFang won't work until re-auth"
                ;;
        esac
        ;;

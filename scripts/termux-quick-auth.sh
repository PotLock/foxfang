...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/foxfang/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "FoxFang Re-Auth" -c "After getting key, run: ssh $SERVER '~/foxfang/scripts/mobile-reauth.sh'" --id foxfang-auth
        ;;
    *)

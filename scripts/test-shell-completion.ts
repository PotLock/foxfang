 ...
 * `foxfang update`. Use it to verify the completion installation flow
...
\`foxfang update\`, \`foxfang doctor\`, and \`foxfang onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# FoxFang Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`foxfang completion --install\` later to enable.`));
    return;
  }

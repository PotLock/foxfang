...
describe("Nix integration (U3, U5, U9)", () => {
  ...
  describe("U5: CONFIG_PATH and STATE_DIR env var overrides", () => {
    it("STATE_DIR defaults to ~/.foxfang when env not set", () => {
    ...
    it("STATE_DIR respects OPENCLAW_HOME when state override is unset", () => {
      ...
      ).toBe(path.join(path.resolve(customHome), ".foxfang"));
    });

    it("CONFIG_PATH defaults to OPENCLAW_HOME/.foxfang/openclaw.json", () => {
      const customHome = path.join(path.sep, "custom", "home");
      expect(
        resolveConfigPathCandidate(
          envWith({
      ...
      ).toBe(path.join(path.resolve(customHome), ".foxfang", "openclaw.json"));
    });

    it("CONFIG_PATH defaults to ~/.foxfang/openclaw.json when env not set", () => {
      expect(
        resolveConfigPathCandidate(
    ...
    it("CONFIG_PATH expands ~ in OPENCLAW_CONFIG_PATH override", async () => {
      await withTempHome(async (home) => {
        expect(
          resolveConfigPathCandidate(
            envWith({ OPENCLAW_HOME: home, OPENCLAW_CONFIG_PATH: "~/.foxfang/custom.json" }),
            () => home,
          ),
        ).toBe(path.join(home, ".foxfang", "custom.json"));
      });
    });
    ...
    it("expands ~ in common path-ish config fields", async () => {
      await withTempHome(async (home) => {
          ...
          JSON.stringify(
            {
              ...
              channels: {
                whatsapp: {
                  accounts: {
                    personal: {
                      authDir: "~/.foxfang/credentials/wa-personal",
                    },
                  },
                },
              },
        ...
        expect(cfg.agents?.list?.[0]?.agentDir).toBe(
          path.join(home, ".foxfang", "agents", "main"),
        );
        expect(cfg.agents?.list?.[0]?.sandbox?.workspaceRoot).toBe(path.join(home, "sandbox-root"));
        expect(cfg.channels?.whatsapp?.accounts?.personal?.authDir).toBe(
          path.join(home, ".foxfang", "credentials", "wa-personal"),
        );
      });
    });
  });

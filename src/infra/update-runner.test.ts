...
describe("runGatewayUpdate", () => {
  ...
  beforeEach(async () => {
    ...
    await fs.writeFile(path.join(tempDir, "foxfang.mjs"), "export {};\n", "utf-8");
  });
  ...
  async function createStableTagRunner(params: {
    ...
    const doctorKey = `${doctorNodePath} ${path.join(tempDir, "foxfang.mjs")} doctor --non-interactive --fix`;
  ...
  it("uses stable tag when beta tag is older than release", async () => {
    ...
    const { runner, calls } = createRunner({
      ...
      [`${doctorNodePath} ${path.join(tempDir, "foxfang.mjs")} doctor --non-interactive --fix`]: {
        stdout: "",
      },
    });
  ...
  it("falls back to npm when pnpm is unavailable for git installs", async () => {
    ...
    const { calls, runCommand } = createGitInstallRunner({
      ...
      doctorCommand: `${process.execPath} ${path.join(tempDir, "foxfang.mjs")} doctor --non-interactive`,
      onCommand: (key) => {
        if (key === "pnpm --version") {
  ...
  it("fails with a clear reason when foxfang.mjs is missing", async () => {
    await setupGitCheckout({ packageManager: "pnpm@8.0.0" });
    await fs.rm(path.join(tempDir, "foxfang.mjs"), { force: true });
